...
class OpenDroidAccessibilityService : AccessibilityService() {
    ...
    // --- Verified Automation (dispatch + confirm + retry once) ---
    //
    // findAndClick()/findAndType() etc. only tell you a node was found and an
    // AccessibilityAction was dispatched — not that the app actually reacted
    // (stale nodes, slow-loading UI, or a click landing on the wrong overlapping
    // view all report "success" today even when nothing happened). These wrappers
    // add a lightweight before/after screen-text comparison and one retry so
    // callers get a truer picture of whether the tap/type actually took effect.

    suspend fun findAndClickVerified(text: String): Boolean {
        val before = getScreenText()
        if (!findAndClick(text)) return false
        delay(450)
        if (getScreenText() != before) return true

        // Nothing visibly changed — the tap may have landed before the UI was
        // ready, or missed. Try once more.
        val retryDispatched = findAndClick(text)
        delay(450)
        return retryDispatched
    }

    suspend fun findAndClickByIdVerified(viewId: String): Boolean {
        val before = getScreenText()
        if (!findAndClickById(viewId)) return false
        delay(450)
        if (getScreenText() != before) return true

        val retryDispatched = findAndClickById(viewId)
        delay(450)
        return retryDispatched
    }

    suspend fun findAndTypeVerified(searchText: String, content: String): Boolean {
        if (!findAndType(searchText, content)) return false
        delay(300)
        val rootNode = rootInActiveWindow ?: return true
        val stillEmpty = rootNode.findAccessibilityNodeInfosByText(searchText)
            .any { it.isEditable && it.text?.contains(content) != true }
        rootNode.recycle()
        if (!stillEmpty) return true

        // Field didn't pick up the text yet — retry once
        val retryDispatched = findAndType(searchText, content)
        delay(300)
        return retryDispatched
    }

    suspend fun clickCoordinatesVerified(x: Float, y: Float): Boolean {
        val before = getScreenText()
        if (!clickCoordinates(x, y)) return false
        delay(450)
        if (getScreenText() != before) return true

        val retryDispatched = clickCoordinates(x, y)
        delay(450)
        return retryDispatched
    }
    ...
    fun longPressCoordinates(x: Float, y: Float): Boolean {
        ...
        // 600ms hold is enough to register as a long-press on virtually all views
    ...
    /**
     * Performs a swipe gesture from (x1,y1) to (x2,y2). Used for scrolling feeds
     * (e.g. Instagram Reels/TikTok-style vertical swipes), dismissing cards, etc.
     * durationMs controls speed — shorter = a flick, longer = a slow drag.
     */
    ...
    /**
     * Convenience swipe by direction across most of the screen, e.g. for feed-style
     * apps like Instagram Reels/YouTube Shorts where scroll actions don't work
     * because the content isn't in a standard scrollable AccessibilityNode.
     */
    fun swipeDirection(direction: String, durationMs: Long = 300): Boolean {
        ...
        val (x1, y1, x2, y2) = when (direction.lowercase()) {
            "up" -> listOf(centerX, height - marginY, centerX, marginY)
            "down" -> listOf(centerX, marginY, centerX, height - marginY)
            "left" -> listOf(width - marginX, centerY, marginX, centerY)
            "right" -> listOf(marginX, centerY, width - marginX, centerY)
            else -> listOf(centerX, height - marginY, centerX, marginY)
        }.map { it as Float }
    ...
    /**
     * Best-effort heuristic to click the first "real" tappable item in a search
     * results screen, skipping the search bar / back button / tab bar which are
     * usually near the very top or very bottom of the screen. Used to make
     * PLAY_MUSIC actually start playback instead of just leaving the search typed.
     */
    fun clickFirstResult(): Boolean {
        val rootNode = rootInActiveWindow ?: return false
        val screenHeight = resources.displayMetrics.heightPixels
        val topExclusion = screenHeight * 0.12f   // skip status bar / search field area
        val bottomExclusion = screenHeight * 0.90f // skip bottom nav bar

        val candidate = findFirstResultNode(rootNode, topExclusion, bottomExclusion)
    ...
    /**
     * Same as clickFirstResult(), but confirms the tap actually changed the
     * screen (e.g. search results → now-playing view) and retries once after
     * a short extra wait if the results list was still loading.
     */
    suspend fun clickFirstResultVerified(): Boolean {
        val before = getScreenText()
        if (!clickFirstResult()) {
            // Results may not have rendered yet — wait a bit longer and retry once
            delay(800)
            if (!clickFirstResult()) return false
            delay(600)
            return true
        }
        delay(600)
    ...
    private fun findFirstResultNode(
        node: AccessibilityNodeInfo,
        topExclusion: Float,
        bottomExclusion: Float
    ): AccessibilityNodeInfo? {
        val bounds = android.graphics.Rect()
        node.getBoundsInScreen(bounds)

        if (node.isClickable && bounds.top >= topExclusion && bounds.bottom <= bottomExclusion && bounds.height() > 0) {
            return AccessibilityNodeInfo.obtain(node)
        }

        for (i in 0 until node.childCount) {
            val child = node.getChild(i) ?: continue
            val found = findFirstResultNode(child, topExclusion, bottomExclusion)
            child.recycle()
