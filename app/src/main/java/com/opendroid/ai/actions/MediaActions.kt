...
class MediaActions @Inject constructor() {
    ...
    private class PlayMusicAction : Action {
        ...
        override suspend fun execute(params: Map<String, String>, context: Context): ActionResult {
            ...
            return try {
                ...
                if (query.isNotEmpty() && (app.lowercase() == "spotify" || app.lowercase() == "youtube")) {
                    // Give the app time to load the search results, then try to actually
                    // tap the first result so playback starts instead of leaving the
                    // query sitting in the search bar.
                    kotlinx.coroutines.delay(2200)
                    val tapped = com.opendroid.ai.accessibility.GenericAppAutomator.clickFirstResult()
                    if (tapped) {
                        ActionResult(true, "Playing '$query' for you!", null)
                    } else {
                        ActionResult(
                            true,
                            "Found '$query' in $app — tap it to start playing (couldn't auto-tap, make sure the Accessibility Service is enabled for OpenDroid).",
                            null
                        )
                    }
                } else {
    ...
    private class OpenYoutubeShortsAction : Action {
        override val name: String = "OPEN_YOUTUBE_SHORTS"
        override suspend fun execute(params: Map<String, String>, context: Context): ActionResult {
            val query = params["query"] ?: ""
            return try {
                val url = if (query.isNotEmpty()) {
                    val encQuery = URLEncoder.encode(query, "UTF-8")
                    "https://www.youtube.com/results?search_query=$encQuery&sp=EgIYAQ%253D%253D"
                } else {
                ...
                if (intent.resolveActivity(context.packageManager) != null) {
                    context.startActivity(intent)
                    ActionResult(true, "Opening YouTube Shorts!", null)
                } else {
                    val browserIntent = Intent(Intent.ACTION_VIEW, Uri.parse(url)).apply {
                        addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
                    }
                    context.startActivity(browserIntent)
                    ActionResult(true, "YouTube app isn't installed, opened Shorts in the browser instead.", null, true)
                }
            } catch (e: Exception) {
                Log.e("OpenYoutubeShorts", "Failed: ${e.localizedMessage}")
                ActionResult(false, null, "Couldn't open YouTube Shorts right now.")
            }
        }
    }
