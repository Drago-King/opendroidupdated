...
class AdvancedControlActions @Inject constructor() {
    ...
    private class LongPressCoordinatesAction : Action {
        override val name: String = "LONG_PRESS_COORDINATES"
        override suspend fun execute(params: Map<String, String>, context: Context): ActionResult {
            val x = params["x"]?.toFloatOrNull() ?: return ActionResult(false, null, "x coordinate is missing or invalid")
            val y = params["y"]?.toFloatOrNull() ?: return ActionResult(false, null, "y coordinate is missing or invalid")
            val success = GenericAppAutomator.longPressCoordinates(x, y)
            return ActionResult(success, if (success) "Long-pressed there!" else "Couldn't long-press at that spot.", null)
        }
    }

    private class SwipeAction : Action {
        override val name: String = "SWIPE"
        override suspend fun execute(params: Map<String, String>, context: Context): ActionResult {
            val x1 = params["x1"]?.toFloatOrNull() ?: return ActionResult(false, null, "x1 coordinate is missing or invalid")
            val y1 = params["y1"]?.toFloatOrNull() ?: return ActionResult(false, null, "y1 coordinate is missing or invalid")
            val x2 = params["x2"]?.toFloatOrNull() ?: return ActionResult(false, null, "x2 coordinate is missing or invalid")
            val y2 = params["y2"]?.toFloatOrNull() ?: return ActionResult(false, null, "y2 coordinate is missing or invalid")
            val duration = params["durationMs"]?.toLongOrNull() ?: 300L
            val success = GenericAppAutomator.swipe(x1, y1, x2, y2, duration)
            return ActionResult(success, if (success) "Swiped!" else "Couldn't swipe there.", null)
        }
    }

    private class SwipeDirectionAction : Action {
        override val name: String = "SWIPE_DIRECTION"
        override suspend fun execute(params: Map<String, String>, context: Context): ActionResult {
            val direction = params["direction"] ?: "up"
            val duration = params["durationMs"]?.toLongOrNull() ?: 300L
            val success = GenericAppAutomator.swipeDirection(direction, duration)
            return ActionResult(success, if (success) "Swiped $direction!" else "Couldn't swipe $direction.", null)
        }
    }
