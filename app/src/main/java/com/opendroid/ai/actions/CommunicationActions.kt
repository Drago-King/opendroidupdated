...
class CommunicationActions @Inject constructor(
    ...
    fun getActions(): List<Action> = listOf(
        ...
        ReadMessagesAction(),
        ReadEmailsAction(),
        OpenInstagramReelsAction(),
        OpenInstagramDirectAction(),
        OpenInstagramProfileAction()
    )
    ...
    // Instagram deep links use Instagram's own verified App Links (https://instagram.com/...).
    // We target the Instagram package directly so it opens straight into the app (not a
    // browser) when Instagram is installed, and fall back to the mobile web page otherwise.
    private inner class OpenInstagramReelsAction : Action {
        override val name: String = "OPEN_INSTAGRAM_REELS"
        override suspend fun execute(params: Map<String, String>, context: Context): ActionResult {
            return openInstagramLink(context, "https://www.instagram.com/reels/", "Opening Instagram Reels!")
        }
    }

    private inner class OpenInstagramDirectAction : Action {
        override val name: String = "OPEN_INSTAGRAM_DIRECT"
        override suspend fun execute(params: Map<String, String>, context: Context): ActionResult {
            return openInstagramLink(context, "https://www.instagram.com/direct/inbox/", "Opening Instagram messages!")
        }
    }

    private inner class OpenInstagramProfileAction : Action {
        override val name: String = "OPEN_INSTAGRAM_PROFILE"
        override suspend fun execute(params: Map<String, String>, context: Context): ActionResult {
            val username = params["username"]
            val url = if (!username.isNullOrBlank()) {
                "https://www.instagram.com/${username.trim().removePrefix("@")}/"
            } else {
                "https://www.instagram.com/"
            }
            return openInstagramLink(context, url, "Opening that Instagram profile!")
        }
    }

    private fun openInstagramLink(context: Context, url: String, successMessage: String): ActionResult {
        return try {
            ...
            if (intent.resolveActivity(context.packageManager) != null) {
                context.startActivity(intent)
                ActionResult(true, successMessage, null)
            } else {
                // Instagram not installed — fall back to the browser
                val browserIntent = Intent(Intent.ACTION_VIEW, Uri.parse(url)).apply {
                    addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
                }
                context.startActivity(browserIntent)
                ActionResult(true, "Instagram app isn't installed, opened it in the browser instead.", null, true)
            }
        } catch (e: Exception) {
            Log.e("OpenInstagram", "Failed: ${e.localizedMessage}")
            ActionResult(false, null, "Couldn't open Instagram right now.")
        }
    }
}
