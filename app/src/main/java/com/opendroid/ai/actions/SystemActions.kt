...
class SystemActions @Inject constructor(
    ...
    // "Guide me" — same vision pipeline as ANALYZE_SCREENSHOT, but with a prompt
    // tuned for step-by-step in-app guidance instead of a plain description, so
    // saying "help me here" / "what do I do now" gets actionable next steps.
    private class GuideMeAction(
        private val visionEngine: dagger.Lazy<com.opendroid.ai.core.agent.VisionEngine>
    ) : Action {
        override val name: String = "GUIDE_ME"
        override suspend fun execute(params: Map<String, String>, context: Context): ActionResult {
            val goal = params["goal"]
            val question = if (!goal.isNullOrBlank()) {
                "The user is inside this app trying to: $goal. Look at what's currently on screen and tell them, briefly and in plain language, exactly what to tap or do next to make progress. Be specific about button/label names visible on screen."
            } else {
                "Look at what's currently on screen and briefly explain, in plain language, what this screen is for and what the user's options are right now. Mention specific button/label names visible on screen."
            }
            return try {
                val guidance = visionEngine.get().analyzeCurrentScreen(question)
                ActionResult(true, guidance, null)
            } catch (e: Exception) {
                Log.e("GuideMe", "Guidance failed: ${e.localizedMessage}")
                ActionResult(false, null, "Couldn't look at the screen to guide you right now.")
            }
        }
    }
